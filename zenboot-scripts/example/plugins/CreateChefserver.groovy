...
class CreateChefserver {

    def grailsApplication

    //@Parameter(name="SERVICEURL", description="The full qualified DNS name from the host")
    def onSuccess = { ProcessContext ctx ->
        ServiceUrl serviceUrl
        if (ctx.parameters['SERVICEURL']) {
          serviceUrl = new ServiceUrl(owner:ctx.host, url:ctx.parameters['SERVICEURL'])
        ...
        ctx.host.addToServiceUrls(serviceUrl)
